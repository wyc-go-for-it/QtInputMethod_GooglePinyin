    ...
    uint16 splid_str_len)
{
    ...
    for (; i < splid_str_len; i++) {
        if (spl_trie.is_half_id(splid_str[i])) {
            searchable->splid_count[i] = spl_trie.half_to_full(splid_str[i],
    ...
    size_t b4_used)
{
    ...
    while (j <= end) {
        ...
        if (memcmp(words, last_hzs, hzs_len << 1) == 0) {
            ...
            uint32 cpy_len = (nchar < kMaxPredictSize ? (nchar << 1) : (kMaxPredictSize << 1))
                - (hzs_len << 1);
...
bool UserDict::load(const char* file, LemmaIdType start_id)
{
    ...
    lemmas = (uint8*)malloc(
        dict_info.lemma_size + (kUserDictPreAlloc * (2 + (kUserDictAverageNchar << 2))));
...
{
    int err = lseek(fd, 4 + dict_info_.lemma_size + (dict_info_.lemma_count << 3)
#ifdef ___PREDICT_ENABLED___
...
{
    int err = lseek(fd, 4 + dict_info_.lemma_size + (dict_info_.lemma_count << 2)
#ifdef ___PREDICT_ENABLED___
            + (dict_info_.lemma_count << 2)
#endif
                            ,
...
begin:
    ...
    syncs_ = syncs_bak;
    if (id == 0 && again == 0) {
        if ((dict_info_.limit_lemma_count > 0 && dict_info_.lemma_count >= dict_info_.limit_lemma_count)
            || (dict_info_.limit_lemma_size > 0 && dict_info_.lemma_size + (2 + (lemma_len << 2)) > dict_info_.limit_lemma_size)) {
