...
{
    return ((size_t)node->homo_idx_buf_off_l + ((size_t)node->homo_idx_buf_off_h << 16));
}
...
bool DictTrie::save_dict(FILE* fp)
{
    ...
    if (fwrite(lma_idx_buf_, sizeof(unsigned char), lma_idx_buf_len_, fp) != lma_idx_buf_len_)
        return false;
...
bool DictTrie::load_dict(FILE* fp)
{
    ...
    if (fread(&top_lmas_num_, sizeof(uint32), 1, fp) != 1 || top_lmas_num_ >= lma_idx_buf_len_)
        return false;
    ...
    splid_le0_index_ = static_cast<uint32*>(malloc(buf_size * sizeof(uint32)));
    ...
    uint32 last_pos = 0;
    for (uint32 i = 1; i < lma_node_num_le0_; i++) {
